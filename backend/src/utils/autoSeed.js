...
async function checkAndSeedProblems() {
    try {
        ...
        if (count > 0) {
            console.log(`Database already has ${count} problems. Skipping auto-seed.`);
            return;
        }

        console.log("No problems found in database. Starting auto-seed...");
        ...
        console.log(`Successfully auto-seeded ${inserted.length} problems!`);
    } catch (err) {
